...
def get_result_f(ident, search, page, limit, db, user):
...
def create_result_f(forms, db, user):
    common = 0
    found = 0
    for form in forms:
...
def get_final_result_f(ident, page, limit, db, user):
    ...
    items = (db.query(FinalResults).filter(ident_filter, FinalResults.user_id == user.id)
             .order_by(FinalResults.id.desc()).offset(page).limit(limit).all())

    return items


def get_final_final_result_f(db, user):
